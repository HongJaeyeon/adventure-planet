...
public class JwtServiceImpl implements JwtService {
	...
	public String getSalt() {
		...
		try {
			propertySources.addLast(new ResourcePropertySource("classpath:hidden.properties"));
			//읽어오기
		} catch(Exception e){
			e.printStackTrace();
		}
		return environment.getProperty("salt");
	}
	...
	public Map<String, Object> get(String key) {
		...
		try {
			claims = Jwts.parser().setSigningKey(getSalt().getBytes("UTF-8")).parseClaimsJws(jwt);
		} catch (Exception e) {
			//			if (logger.isInfoEnabled()) {
			//				e.printStackTrace();
			//			} else {
